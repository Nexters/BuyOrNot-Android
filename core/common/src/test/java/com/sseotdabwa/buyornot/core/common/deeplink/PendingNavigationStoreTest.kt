...
class PendingNavigationStoreTest {
    private val store = PendingNavigationStore()

    @Test
    fun `set한_목적지를_그대로_들고_있는다`() {
        store.set(PendingNavigation(NavigationDestination.FEED_CREATE))

        assertEquals(NavigationDestination.FEED_CREATE, store.pending.value?.destination)
    }
    ...
    fun `여러_번_읽어도_소비되지_않는다`() {
        ...
        store.set(PendingNavigation(NavigationDestination.HOME))

        assertEquals(NavigationDestination.HOME, store.pending.value?.destination)
        assertEquals(NavigationDestination.HOME, store.pending.value?.destination)
        assertEquals(NavigationDestination.HOME, store.pending.value?.destination)
    }
    ...
    fun `나중에_들어온_알림이_이전_목적지를_덮어쓴다`() {
        ...
        store.set(PendingNavigation(NavigationDestination.FEED_DETAIL, feedId = 169L, notificationId = 42L))

        assertEquals(NavigationDestination.FEED_DETAIL, store.pending.value?.destination)
