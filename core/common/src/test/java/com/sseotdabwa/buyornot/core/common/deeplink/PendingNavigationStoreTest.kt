...
class PendingNavigationStoreTest {
    ...
    fun `set한_목적지를_그대로_들고_있는다`() {
        store.set(PendingNavigation(EntryDestination.FEED_CREATE))

        assertEquals(EntryDestination.FEED_CREATE, store.pending.value?.destination)
    }
    ...
    fun `여러_번_읽어도_소비되지_않는다`() {
        ...
        store.set(PendingNavigation(EntryDestination.HOME))

        assertEquals(EntryDestination.HOME, store.pending.value?.destination)
        assertEquals(EntryDestination.HOME, store.pending.value?.destination)
        assertEquals(EntryDestination.HOME, store.pending.value?.destination)
    }
    ...
    fun `나중에_들어온_알림이_이전_목적지를_덮어쓴다`() {
        ...
        store.set(PendingNavigation(EntryDestination.FEED_DETAIL, feedId = 169L, notificationId = 42L))

        assertEquals(EntryDestination.FEED_DETAIL, store.pending.value?.destination)
